	...
	public function getJsonEstoque($estoque)
	{
		...
		foreach ($estoque as $estoqueProd):
			...
			$aux = array(
						...
						'foto'=> $foto,
						'qtdtotal'=> $dataformat->formatar($estoqueProd->getQuantidadeTotal(),'decimalinteiro').' '.$estoqueProd->getProduto()->getUnidadeMedidaParaVenda()->getUnidadeMedida()->getNome(),
						...
						'linkAlterarLimites' => URL."estoque/prateleira/gerenciar/limitar",
						'acoes'=> "",
				      	'lotes'=> array()
				    );
			$arrLotes = array();
			...
			foreach ($estoqueProd->getLotes() as $lotes){
		        ...
		        $aux2 = array(
							...
							'linkemprateleirar' => URL."estoque/prateleira/gerenciar/emprateleirar",
